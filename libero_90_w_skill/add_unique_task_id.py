...
def add_task_id(path, out_dir):
    ...
    for task_id in range(num_tasks_in_suite):
        ...
        orig_data_path = os.path.join(path, f"{task.name}_demo.hdf5")
        out_path = os.path.join(out_dir, f"{task.name}_demo.hdf5")
        os.makedirs(out_dir, exist_ok=True)
        print("processing", orig_data_path)
        with h5py.File(orig_data_path, "r") as F_in, h5py.File(out_path, "w") as F_out:
            F_in.copy("data", F_out)
            for key in F_in["data"]:
                F_out["data"][key].create_dataset("task_id", data=task_id)


if __name__ == "__main__":
    input_dir = "/mnt/shared_data/dataset/dataset_hdf5/libero_90_no_noops/"
    out_dir_task_id = "/mnt/shared_data/dataset/dataset_hdf5/libero_90_with_task_id"
    out_dir_skill_id = "/mnt/shared_data/dataset/dataset_hdf5/libero_90_with_skill_id"
    

    add_task_id(input_dir, out_dir_task_id)
        
    for path in glob.glob(os.path.join(out_dir_task_id, "*.hdf5")):
        add_skill_id(path, out_dir_skill_id)
