...
def _generate_examples(paths) -> Iterator[Tuple[str, Any]]:
    ...
    def _parse_example(episode_path, demo_id):
        ...
        with h5py.File(episode_path, "r") as F:
            ...
            skill_id = F['data'][f"demo_{demo_id}"]["skill_id"][()]
            task_id = F['data'][f"demo_{demo_id}"]["task_id"][()]
        ...
        for i in range(actions.shape[0]):
            episode.append({
                ...
                'task_id': task_id, # use random uuid to unique identify the task
                'skill_id': skill_id,
            })
...
class LIBERO_90_W_SKILL(MultiThreadedDatasetBuilder):
    ...
    N_WORKERS = 16             # number of parallel workers for data conversion (reduced for CPU mode)
    MAX_PATHS_IN_MEMORY = 40   # number of paths converted & stored in memory before writing to disk (reduced for CPU mode)
    ...
    def _split_paths(self):
        ...
        return {
            "train": glob.glob("/mnt/shared_data/dataset/dataset_hdf5/libero_90_with_skill_id/*.hdf5"),
        }
